...
/** Parse a dotenv file. Quotes stripped. Comments and blank lines ignored. */
...
/** `.env` then `.env.local` from `root`. Missing files are ignored. */
export function readDotEnv(root) {
  const env = {};
  for (const name of [".env", ".env.local"]) {
    try {
      Object.assign(env, parseDotEnv(readFileSync(join(root, name), "utf8")));
    } catch {
      /* optional */
    }
  }
